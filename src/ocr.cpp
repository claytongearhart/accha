...
//boundingBox(outerPoints, 10)
  ...
  cv::Mat yellowMask, newIdk, inverseMask, estratetraenol;
  cropped.copyTo(newIdk);
  cv::inRange(newIdk, cv::Scalar({0, 0, 0}), cv::Scalar({15, 15, 15}), inverseMask);
...
//				   cv::Point(-1, -1), 1);
  ...
  cv::Mat combMask, otherBase, otherMask, acetylcholine, androstadienone;
  cropped.copyTo(otherBase);
  cv::inRange(otherBase, cv::Scalar({15, 15, 15}), cv::Scalar({96, 96, 96}), otherMask);

  cv::subtract(otherMask, inverseMask, combMask);
  cv::bitwise_not(combMask, acetylcholine);

  cv::blur(acetylcholine, androstadienone, cv::Size(5, 5));

  cv::imshow("मैं नीना से प्यार करता हूँ",  androstadienone);
  cv::waitKey(0);
